...
public class Solution {
    ...
    */
    ...
    public boolean isSuccessful(GameModel model)
    {
        ...
        for(int i = 0; i < height ; i++){
            for(int j = 0; j < width; j++) {
                if(!correctNeighborhood(model,i,j)){
                    return false;
                }
            }
        }
    ...
    */
    public boolean stillPossible(boolean nextValue, GameModel model)
    {
        ...
        //boolean isOn = model.isON(i,j);

        if((i > 0) && (!correctNeighborhood(model,i-1,j))){
            possible = false;
        }
        if(possible && (i == (height-1))) {
            if((j > 0) && (!correctNeighborhood(model,i,j-1))){
                possible = false;
            }
            if(possible && (j == (width-1))&& (!correctNeighborhood(model,i,j))){
                possible = false;            
            }
        }
        board[i][j] = before;
    ...
    */
    public boolean finish(){
        System.out.println("Old Finish called");
...
*/
    ...
    public boolean finish(GameModel model)
    {
        ...
        while(currentIndex < height*width) {
            //isOn = model.isON(i, j);
            if(i < height - 1 ) {
                setNext(!correctNeighborhood(model,i-1,j));
                i = currentIndex/width;
                j = currentIndex%width;
            } else { //last raw
                if(j == 0){
                   ...
                   if((height > 1) && correctNeighborhood(model,i-1,j) != correctNeighborhood(model,i,j-1)){
                     return false;
                   }
                   setNext(!correctNeighborhood(model,i,j-1));
                } 
                i = currentIndex/width;
                j = currentIndex%width;
            }
        ...
        if(!correctNeighborhood(model,height-1,width-1)){
            return false;
        }
        // here we should return true because we could
        // successfully finish the board. However, as a
    ...
    private boolean correctNeighborhood(GameModel model, int i, int j)
    {
        if(i>=0 && j>=0 && i<model.getHeight() && j<model.getWidth()) {
            if (model.isON(i, j)) {
                return !oddNeighborhood(i, j);
            } else {
                return oddNeighborhood(i, j);
            }
        }else return false;
