...
public class LightsOut {
    ...
    public static Solution solveShortest(GameModel model)
    {
        ...
        int smallestSolutionIndex = 0;

        for (int i = 0; i < solutions.size(); i++)
        {
            if (solutions.get(i).getSize() < solutions.get(smallestSolutionIndex).getSize())
            {
                smallestSolutionIndex = i;
            }
        }

        return (solutions.size() > 0) ? solutions.get(smallestSolutionIndex) : null;
    }
